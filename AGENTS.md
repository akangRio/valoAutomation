...
2. **Database State Machine**: 
   - SQLite (`/packages/database/state.db`) is the absolute source of truth for execution states. 
   - Do NOT attempt to run standard high-concurrency Node managers. Jobs are executed in strict sequential order (concurrency = 1) due to local Windows host resource limits during gaming.
3. **No Code Churn**: 
...
| :--- | :--- |
| `apps/` | Main execution daemons (`orchestrator`, `capture-monitor`, `video-renderer`, etc.) |
...
| `infrastructure/` | Local configuration templates, Windows Scheduler configurations, and setups |
...
2. **Step 2: Add Environment Schemas**: Use Zod or Pydantic to validate environment variables. Fail early and loudly at process boot if variables are missing.
...
- **[DATABASE.md](file:///home/student_03_c5def29c309c/valoAutomation/docs/DATABASE.md)**: SQLite connection strings, WAL pragmas, and schema queries.
